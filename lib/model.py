...
class BaseModel():
    ...
    def __init__(self, opt, dataloader):
        ...
        self.trn_dir = os.path.join(self.opt.outf, self.opt.name, str(self.opt.perc_pullation), 'train')
        self.tst_dir = os.path.join(self.opt.outf, self.opt.name, str(self.opt.perc_pullation), 'test')
    ...
    def save_weights(self, epoch):
        ...
        weight_dir = os.path.join(self.opt.outf, self.opt.name, str(self.opt.perc_pullation), 'train', 'weights')
    ...
    def train(self):
        ...
        for self.epoch in range(self.opt.iter, self.opt.niter):
            ...
            res = self.test(self.epoch==self.opt.iter or self.epoch==self.opt.niter-1)
    ...
    def test(self, display):
        ...
        with torch.no_grad():
            ...
            if self.opt.load_weights:
                path = "./output/{}/{}/{}/train/weights/netG.pth".format(self.name.lower(), self.opt.dataset, str(self.opt.perc_pullation))
            ...
            for i, data in enumerate(self.dataloader['test'], 0):
                ...
                # Save test images.
                if self.opt.save_test_images and display and print:
                    print=False
                    dst = os.path.join(self.opt.outf, self.opt.name, str(self.opt.perc_pullation), 'test', 'images')
    ...
    def test_test_one_imageflv(self, data):
        with torch.no_grad():
            # Load the weights of netg and netd.
            path = "./output/{}/{}/{}/train/weights/netG.pth".format(self.name.lower(), self.opt.dataset, str(self.opt.perc_pullation))
            pretrained_dict = torch.load(path)['state_dict']
            ...
            self.fake, latent_i, latent_o = self.netg(self.input)
            error = torch.mean(torch.pow((latent_i-latent_o), 2), dim=1)
            err = error.reshape(error.size(0))
        return self.fake, err
    ...
    def latent_plot(self, latent, y, n, savename=""):
        plt.figure(figsize=(6, 5))
        plt.scatter(latent[:, 0], latent[:, 1], c=y, \
            marker='o', edgecolor='none', cmap=self.discrete_cmap(n, 'jet'))
